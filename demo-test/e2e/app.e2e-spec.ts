...
describe('Starting tests for demo-test', function() {
  ...
  it('website title should be demo-test', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('demo-test');
    })
  });

  it('network-name should be demo-test-network@0.2.6-deploy.0',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('demo-test-network@0.2.6-deploy.0.bna');
    });
  });

  it('navbar-brand should be demo-test',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('demo-test');
    });
  });
